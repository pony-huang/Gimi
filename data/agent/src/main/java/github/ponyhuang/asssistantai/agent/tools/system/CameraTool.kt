...
class CameraTool @Inject constructor(private val queue: IntentActionQueue) {
    @Tool(name = "capture_photo", description = "Opens the system camera to take a photo.", requireConfirmation = true) fun capturePhoto(): Map<String, Any> = queue.request(
        title = "Take photo", summary = "Open the system camera to take a photo.",
        intent = Intent(MediaStore.ACTION_IMAGE_CAPTURE),
    )

    @Tool(name = "capture_video", description = "Opens the system camera to record a video.", requireConfirmation = true) fun captureVideo(): Map<String, Any> = queue.request(
