...
class SettingsNavigationTool @Inject constructor(private val queue: IntentActionQueue) {
    @Tool(name = "open_settings", description = "Opens the requested system settings page.") fun openSettings(@Param("Settings page: general, wireless, airplane, wifi, bluetooth, date, locale, input, display, security, location, storage.") page: String): Map<String, Any> {
        ...
        val action = when (normalized) {
            ...
            "wifi" -> Settings.ACTION_WIFI_SETTINGS; "bluetooth" -> Settings.ACTION_BLUETOOTH_SETTINGS; "date" -> Settings.ACTION_DATE_SETTINGS
            "locale" -> Settings.ACTION_LOCALE_SETTINGS; "input" -> Settings.ACTION_INPUT_METHOD_SETTINGS; "display" -> Settings.ACTION_DISPLAY_SETTINGS
            "security" -> Settings.ACTION_SECURITY_SETTINGS; "location" -> Settings.ACTION_LOCATION_SOURCE_SETTINGS; "storage" -> Settings.ACTION_INTERNAL_STORAGE_SETTINGS
