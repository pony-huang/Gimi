...
class MessagingTool @Inject constructor(private val queue: IntentActionQueue) {
    @Tool(name = "compose_message", description = "Opens a messaging app with a draft SMS message. The user reviews and sends it.") fun composeMessage(@Param("Recipient phone number, optional.") phoneNumber: String? = null, @Param("Message text.") text: String): Map<String, Any> {
