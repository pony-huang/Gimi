...
你填自己的 API Key。Gimi 让模型能操作你的设备：闹钟、日历、媒体播放、屏幕亮度、你指定的文件夹，以及你接入的外部工具。不绑定厂商，不需要订阅。每一步操作都先经过你确认。

## 功能

### 聊天

流式回复，支持拍照或相册发图。聊到一半可以换模型。最多同时跑三个任务。

### 语音

点按说话，或者设唤醒词后台待命。唤醒词识别在本机完成，中文模型已内置，英文模型需下载。
...
连接远程 MCP 服务器（SSE 或 Streamable HTTP）扩展能力。可手动添加，也可粘贴 JSON 配置批量导入。

### 技能

从链接或本地 ZIP 安装技能包。技能就是一份指引和资源，装好后需要用的时候助手会自动调用。
...
支持 OpenAI 兼容和 Anthropic 协议，其他服务填自定义地址也能用。

还可配置：快速模型（会话标题）、语音识别模型、语音合成模型。
